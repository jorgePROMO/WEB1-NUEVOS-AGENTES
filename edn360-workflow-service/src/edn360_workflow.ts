...
const E7TrainingPlanAssemblerSchema = z.object({ client_training_program: z.object({ title: z.string(), summary: z.string(), goal: z.string(), training_type: z.string(), days_per_week: z.any(), session_duration_min: z.any(), weeks: z.any(), sessions: z.array(z.object({ id: z.string(), name: z.string(), focus: z.array(z.string()), blocks: z.array(z.object({ id: z.string(), primary_muscles: z.array(z.string()), secondary_muscles: z.array(z.string()), exercises: z.array(z.object({ order: z.any(), db_id: z.string(), series: z.any(), reps: z.string(), rpe: z.any() })) })), session_notes: z.array(z.string()) })), general_notes: z.array(z.string()) }) });
const E75TrainingPlanEnricherSchema = z.object({ client_training_program_enriched: z.object({ title: z.string(), summary: z.string(), goal: z.string(), training_type: z.string(), days_per_week: z.any(), session_duration_min: z.any(), weeks: z.any(), sessions: z.array(z.object({ id: z.string(), name: z.string(), exercises: z.array(z.object({ order: z.any(), db_id: z.string(), name: z.string(), series: z.any(), reps: z.string(), rpe: z.any(), video_url: z.string() })), session_notes: z.array(z.string()) })), general_notes: z.array(z.string()) }) });
...
export const runWorkflow = async (workflow: WorkflowInput) => {
  ...
  return await withTrace("EDN360 – Entreno v1", async () => {
    ...
    conversationHistory.push(...e2ParseQuestionnaireResultTemp.newItems.map((item) => item.rawItem));
    ...
    const e7TrainingPlanAssemblerResultTemp = await runner.run(
    ...
    conversationHistory.push(...e75TrainingPlanEnricherResultTemp.newItems.map((item) => item.rawItem));
