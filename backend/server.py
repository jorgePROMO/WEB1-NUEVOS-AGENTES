...
async def google_calendar_callback(code: str):
    ...
    try:
        ...
        # Guardar tokens en la configuración del admin
        # Asumimos que solo el admin principal usa el calendario
        admin_email = "ecjtrainer@gmail.com"
...
async def create_event(request: Request, event_data: dict):
    ...
    {
        "client_id": "user123",
        "title": "Revisión mensual con María",
    ...
    try:
        ...
        if not config or not config.get("google_tokens"):
            raise HTTPException(
                status_code=400,
                detail="Google Calendar no está conectado. Conecta primero desde Configuración."
            )
        
        tokens = config["google_tokens"]
        
        # Obtener email del cliente si se proporciona
        client_email = None
        if event_data.get("client_id"):
            client = await db.users.find_one({"id": event_data["client_id"]})
            if client:
        ...
        # Guardar referencia del evento en la BD
        event_ref = {
        ...
        await db.calendar_events.insert_one(event_ref)
...
async def delete_event(request: Request, event_id: str):
    ...
    try:
        ...
        if success:
            # Eliminar también de nuestra BD
            await db.calendar_events.delete_one({"google_event_id": event_id})
            return {"success": True}
        else:
            raise HTTPException(status_code=500, detail="Failed to delete event")
